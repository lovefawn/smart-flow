      ...
      <template #bodyCell="{ column, record, text }">
        ...
        <template v-else-if="column.dataIndex === 'action'">
          <div class="smart-table-operate" v-if="!record.deletedFlag">
            <a-button type="link" v-if="!record.instanceId" @click="addOrUpdate(record.noticeId)" v-privilege="'oa:notice:update'">编辑</a-button>
            <a-button type="link" v-if="!record.instanceId" @click="onDelete(record.noticeId)" v-privilege="'oa:notice:delete'" danger>删除</a-button>
            <a-button type="link" v-if="record.instanceId&&record.flowStatus&&record.flowStatus!=8" @click="approval(record.noticeId)" v-privilege="'oa:notice:approve'" >审批</a-button>
          </div>
        </template>
      </template>
    </a-table>
