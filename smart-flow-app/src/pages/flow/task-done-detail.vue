    ...
    <view class="content-container">
      ...
      <view v-else-if="taskDetail" class="detail-container">
        ...
        <view v-if="type !== 'done' && type !== 'copy'" class="action-buttons">
          ...
          <button class="btn btn-secondary" @click="viewProcess">查看流程</button>
        </view>
...
const loadTaskDetail = async () => {
  ...
  try {
    loading.value = true;
    const res = await instanceApi.getDoneTaskById(props.taskId);
...
const viewProcess = () => {
  uni.navigateTo({
    url: `/pages/flow/process-view?instanceId=${props.instanceId}`
  });
};
