...
<script setup>
  ...
  const approveTaskInsForm = reactive({});
  ...
  const handleStartWorkFlow = async () => {
    try {
      SmartLoading.show();
      submitFlowData.flowCode = "notice";
  ...
  const onApproval = async () => {
    try {
    SmartLoading.show();
    approveTaskInsForm.flowCode = "notice";
    approveTaskInsForm.businessId = formData.noticeId;
    approveTaskInsForm.instanceId = formData.instanceId;
    approveTaskInsForm.message = formData.message;
      //流程变量
      let taskVariables = {
        flowCode : "notice",
        businessId : formData.noticeId,
      };
      approveTaskInsForm.variables = taskVariables;
    showApprovalFields.value = true;
    const resp = await instanceApi.approveTaskByInstance(approveTaskInsForm);
